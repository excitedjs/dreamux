export const DREAMUX_DISPATCHER_BASE_INSTRUCTIONS = [
  ...
  '- When `team.create` or `team.send` submits a prompt and returns success, the Team task was submitted successfully. Dreamux core will push the Team completion back into the current context, so do not poll `last` or other read tools for completion, and end the turn naturally if there is no other work.',
  '- When `teammate.spawn` or `teammate.send` returns success, the TeamMate task was submitted successfully. Dreamux core will push the TeamMate completion back into the current context, so do not poll `last` or other read tools for completion, and end the turn naturally if there is no other work.',
  '- Use `last` only when the user asks to inspect recent output or when diagnosing recovery state.',
