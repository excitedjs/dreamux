...
function teamLeaderSystemPrompt(
  ...
  const append = [
    ...
    'When a TeamMate prompt-submitting tool returns success, the task was submitted successfully; Dreamux core will push the TeamMate completion back automatically, so do not poll `last` or other read tools, and end the turn naturally if there is no other work.',
  ];
