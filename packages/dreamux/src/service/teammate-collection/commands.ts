 ...
 * services; these definitions own the payload contract and the public failure
 * vocabulary.
 */
...
export function teammateCommands(
  ...
  > = {
    ...
    parse(payload) {
      ...
      return {
        name: mustString(params, 'name_prefix'),
    ...
    async execute(context, input) {
      ...
      try {
        return await dispatcher.teammates.spawn(spawnInput);
      } catch (err) {
  ...
  > = {
    ...
    async execute(context, input) {
      try {
        return await mustDispatcher(host, context).teammates.send({
          name: input.name,
          prompt: input.prompt,
  ...
  > = {
    ...
    async execute(context, input) {
      try {
        return await mustDispatcher(host, context).teammates.close({
          name: input.name,
          note: input.note,
        });
      } catch (err) {
  ...
  > = {
    ...
    async execute(context, input) {
      return mustDispatcher(host, context).teammates.history(input.query);
    },
  };
  ...
  > = {
    ...
    output: objectSchema({ teammate: OBJECT }, ['teammate']),
    parse(payload) {
      return { name: mustString(commandPayload(payload), 'name') };
    },
    async execute(context, input) {
      return {
  ...
  const last: CoreCommandDefinition<'teammate.last', LastInput, AgentEntityLastResult> = {
    ...
    parse(payload) {
      ...
      try {
        ...
        throw new ValidationError('teammate.last limit must be an integer in 1..200');
      }
      const includeTools = optionalBooleanField(params, 'include_tools')['include_tools'];
      return {
        name: mustString(params, 'name'),
        limit,
        cursor: optionalString(params, 'cursor'),
        includeTools: includeTools ?? null,
      };
    },
    async execute(context, input) {
      try {
        return await mustDispatcher(host, context).teammates.last(input.name, {
          ...(input.limit !== null ? { limit: input.limit } : {}),
          ...(input.cursor !== null ? { cursor: input.cursor } : {}),
          ...(input.includeTools !== null ? { includeTools: input.includeTools } : {}),
        });
      } catch (error) {
  ...
  > = {
    ...
    parse: () => ({}),
    async execute(context) {
      return mustDispatcher(host, context).teammates.getCapabilities();
    },
  };
