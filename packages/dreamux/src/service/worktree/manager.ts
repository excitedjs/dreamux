...
export class WorktreeManager {
   ...
   * itself when disabled. Neither mode creates a git worktree. The dispatcher
   * cwd need not be a git repo — no git command runs — so `source_repo` is
   * reported as null even if the directory happens to sit inside a repo. The
   * `.workspace/` boundary is only created for the isolated mode.
   */
