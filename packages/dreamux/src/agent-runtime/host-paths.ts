 ...
 * artifacts land — the per-runtime state root, the central log tree, and the
 ...
 * a teammate's state runtime dir stays nested under its OPERATOR dispatcher (not
 * its composite runtime id), so a teammate context closes over the operator id
 * rather than keying off the passed runtime id.
 */
 ...
 * state files under `<stateRoot>/<id>/`, logs under the central tree, sockets in
 * the preference-ordered volatile dirs.
 */
 ...
 * dir stays grouped under the operator dispatcher (so a teammate's `mcp.json`
 * lands in `<operator>/runtime/<name>/`), independent of the composite runtime
 * id the launcher assigns. The scratch root sits beside the `teammate/` and
 * `team/` blind-scan collections (issue #233), never inside them. Logs and
 * sockets share the host-wide roots.
 *
