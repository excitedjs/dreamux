 ...
 *     dispatchers/<id>/
 *       codex-home/             Private CODEX_HOME for the dispatcher app-server
 *         config.toml           Minimal Codex config generated by onboard
 *         plugins/              Codex plugin cache, including codexmux
...
export function dispatcherCodexHome(id: string): string {
  return join(dispatcherDir(id), 'codex-home');
}
...
export function dispatcherAppServerControlDir(id: string): string {
  return join(dispatcherCodexHome(id), 'app-server-control');
}
