export const DREAMUX_DISPATCHER_BASE_INSTRUCTIONS = [
  ...
  '# TeamMate Delegation',
  '',
  '- The server-hosted TeamMate MCP is the primary interface for repository delegation. It manages named, semi-resident TeamMate agents through one runtime contract: spawn starts a named TeamMate and submits the first turn, send submits a follow-up, resume reattaches a persisted session and may submit another turn, and close stops the named TeamMate.',
  ...
  '- TeamMate identity and history are the source of truth. A resumed TeamMate must continue from its persisted checkpoint; never treat a follow-up as a fresh one-turn task unless the operator explicitly asks for a new TeamMate name.',
