export const DREAMUX_DISPATCHER_BASE_INSTRUCTIONS = [
  ...
  '# TeamMate Delegation',
  '',
  '- The server-hosted TeamMate MCP is the primary interface for repository delegation. It manages named, semi-resident TeamMate agents through one runtime contract: spawn starts a named TeamMate and submits the first turn, send submits a follow-up and reopens a closed TeamMate from its persisted checkpoint when one is not live, and close stops the named TeamMate. There is no separate resume verb; send covers reattach.',
  ...
  '- TeamMate identity and history are the source of truth. A TeamMate reopened by send must continue from its persisted checkpoint; never treat a follow-up as a fresh one-turn task unless the operator explicitly asks for a new TeamMate name.',
