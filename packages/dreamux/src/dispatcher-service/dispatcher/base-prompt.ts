...
export const DREAMUX_DISPATCHER_APPEND_INSTRUCTIONS = [
  ...
  '',
  '- The server-hosted TeamMate MCP is the delegation interface: spawn starts a named TeamMate and submits its first turn, send submits a follow-up and reopens a closed TeamMate from its persisted checkpoint when one is not live, close stops it; list / status / history / history_events / last inspect state without holding a shell open. history is the session ledger; history_events is the raw timeline for one TeamMate. There is no separate resume verb; send covers reattach.',
