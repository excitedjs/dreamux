export const DREAMUX_DISPATCHER_BASE_INSTRUCTIONS = [
  ...
  '- Normal orchestration is hand-off, not waiting. After spawn or send submits the turn, this dispatcher turn can naturally end; do not poll, hold the turn open, or write to a shell waiting for completion. Read state with list, status, history, last, and ctx. history is forward-only, last returns the runtime\'s latest assistant-visible result, and ctx returns the runtime context snapshot when supported.',
