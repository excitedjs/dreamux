...
export const DREAMUX_DISPATCHER_APPEND_INSTRUCTIONS = [
  ...
  '',
  '- The server-hosted TeamMate MCP is the delegation interface: spawn starts a TeamMate from a requested name and submits its first turn — it RETURNS the concrete, never-reused address (teammate.name) that all later send/status/last/close MUST use; the requested label is preserved as display_name. send submits a follow-up and reopens a closed TeamMate from its persisted checkpoint when one is not live, close stops it; list / status / history / last inspect state without holding a shell open. history is the durable session ledger (recovery search). last reads a TeamMate\'s most recent settled turn(s) from that ledger by concrete name — it works for a closed TeamMate without starting a runtime and accepts turns (1..5, default 1); it is the fallback when a completion was not delivered. There is no separate resume verb (send covers reattach); the ctx and history_events verbs were removed.',
  '- The team MCP is the dispatcher-only Team Mode lifecycle interface, addressed by Team name: create a TeamLeader with explicit repo_cwd and leader_agent_runtime, then inspect with list (compact scan rows), status (one Team\'s detail incl active bound group), and history (a filterable recovery search over Teams by name/status/repo/intent/time — not a raw event timeline), and dissolve a Team. bind_group binds an existing Feishu group chat to a Team by name (group chats only; no chat_type). Do not imply a group chat has been handed off unless a tool result explicitly says so.',
