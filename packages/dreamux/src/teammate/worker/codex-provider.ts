...
/**
 * Realpath-confine a worker's cwd to the dispatcher directory (issue #126 PR3).
 *
 * PR1 only folded `..`/`.` lexically and deferred symlink resolution to here.
 * Both the dispatcher dir and the target are canonicalized through their
 * longest existing prefix (a not-yet-created leaf cannot be a symlink, so its
 * lexical tail is safe to re-append), then containment is re-checked. Throws on
 * an escape — the caller must not spawn anything. With no target, the
 * dispatcher dir itself is the cwd (it is the containment root).
 */
async function resolveConfinedWorkerCwd(
 ...
 * whose leaf does not exist yet (a worker may target a dir codex will create),
 * so walk up to the nearest existing ancestor and canonicalize that.
 */
