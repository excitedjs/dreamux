...
| `dispatcher-service/dispatcher/` | the dispatcher agent as a contained `TeammateService` (`agent.ts` factory) + the live `ChannelSessions` (`channel-sessions.ts`) + the TeamLeader channel-egress gate (`channel-tool-auth.ts`) + role MCP descriptor builders (`mcp-descriptors.ts`) + dispatcher base prompt + runnable-channel guard. Agent runtime lifecycle (start/resume/stop) lives in the shared `TeammateService`; `DispatcherService` (`dispatcher-instance.ts`) keeps channel sessions, restart-notice injection, MCP assembly, and completion routing | the dispatcher *has an* agent (Phase 5, #233); there is no separate `DispatcherRuntimeService` |
| `dispatcher-service/teammate/` | `TeammateCollection` + `TeammateService` entity + `CompletionRouter` + identity-store + runtime-state + types + teammate MCP descriptor | agent-centric teammates (no `task`): spawn/send/close + forward-only history (send reopens a closed teammate; no separate `resume` verb, #155) |
