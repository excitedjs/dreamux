...
| `service/dispatcher-service/` | the per-dispatcher aggregate (`index.ts` = `DispatcherService`) + its agent-side parts: the dispatcher agent as a contained `TeammateService` (`agent.ts` factory), the live `ChannelSessions` (`channel-sessions.ts`), the TeamLeader channel-egress gate (`channel-tool-auth.ts`), role MCP descriptor builders (`mcp-descriptors.ts`), dispatcher base prompt, runnable-channel guard, workspace policy (`workspace.ts`), and `errors.ts`. Agent runtime lifecycle (start/resume/stop) lives in the shared `TeammateService`; `DispatcherService` keeps channel sessions, restart-notice injection, MCP assembly, and completion routing. `service/dispatchers/` holds the process-level `Dispatchers` collection | the dispatcher *has an* agent (Phase 5, #233); there is no separate `DispatcherRuntimeService` |
| `service/teammate-collection/` (+ `service/teammate-service/`, `service/completion-router/`) | `TeammateCollection` (collection: stores, factory paths, router registration) + `TeammateService` entity + `CompletionRouter` + identity/turns stores + read-model + runtime-state + types + teammate MCP descriptor. `service/team-collection/` + `service/team-service/` hold `TeamCollection` / `TeamService`; `service/worktree/`, `service/channel-binding/`, `service/legacy-state.ts` are shared helpers at the module root | agent-centric teammates (no `task`): spawn/send/close + forward-only history (send reopens a closed teammate; no separate `resume` verb, #155) |
