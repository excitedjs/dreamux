...
/**
 * An in-memory stand-in for the platform COT surface.
 *
...
export function createFakeCotClient(
  ...
  const client = createFeishuCotClient(
    {
      async request(input: unknown): Promise<unknown> {
        ...
        if (isCreate(request)) {
          const id = nextId;
          nextId += 1;
          return { code: 0, data: { cot_id: `cot-${id}`, message_id: `om-cot-${id}` } };
        }
        return { code: 0 };
      },
  ...
  const fake: FakeCotClient = {
    ...
    createdCotIds: () =>
      requests.filter(isCreate).map((_, index) => `cot-${index + 1}`),
    eventsFor(cotId: string): DecodedCotEvent[] {
      return requests
        .filter((request) => isAppend(request) && request.data?.['cot_id'] === cotId)
        .flatMap(wireEvents)
        .map((event) => ({
...
export async function settleCot(rounds = 8): Promise<void> {
