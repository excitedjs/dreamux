...
export function toolCallStartEvents(event: CotToolCallActivity): FeishuCotEventInput[] {
  ...
  const events = [
    ...
    // its arguments by operator ruling, 2026-09-04.
    checkedEvent({
 ...
 * 2026-09-04 — text output stays text, and only what parses as JSON goes into
 * a code segment). The whole text is parsed first and cut last, so a cut never
 * decides what a value was.
