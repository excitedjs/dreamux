...
describe('handleProtocolEvent token usage', () => {
  it('emits the cumulative usage snapshot immediately before native end', () => {
    ...
    expect(events.map((fact) => fact.kind)).toEqual(['token.usage', 'turn.ended', 'turn.ended']);
  });

  it('reports background result usage without any submitted command and a null context', () => {
    const h = makeHarness();
    h.fire(resultEvent(outcome({ tokenUsage: { inputTokens: 10, outputTokens: 5 }, contextTokens: null }), []));
    expect(h.activityEvents[0]).toMatchObject({
      kind: 'token.usage', inputTokens: 10, outputTokens: 5, context: null,
    });
