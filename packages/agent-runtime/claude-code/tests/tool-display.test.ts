...
describe('toolDisplay', () => {
  it('leads a Bash call with its stated description and keeps the command line as the invocation', () => {
    expect(toolDisplay('Bash', { command: 'git status --short', description: 'Show working tree status' }))
      .toEqual({ action: 'run', summary: 'Show working tree status', invocation: 'git status --short', files: [] });
  });
  ...
  it('labels file tools by the path they touch, which is also their file list, with nothing to invoke', () => {
    expect(toolDisplay('Read', { file_path: '/repo/src/a.ts', offset: 10 }))
      .toEqual({ action: 'read', summary: '/repo/src/a.ts', invocation: null, files: ['/repo/src/a.ts'] });
    expect(toolDisplay('Edit', { file_path: '/repo/src/a.ts', old_string: 'x', new_string: 'y' }))
      .toEqual({ action: 'edit', summary: '/repo/src/a.ts', invocation: null, files: ['/repo/src/a.ts'] });
    expect(toolDisplay('NotebookEdit', { notebook_path: '/repo/n.ipynb', new_source: '' }))
      .toEqual({ action: 'edit', summary: '/repo/n.ipynb', invocation: null, files: ['/repo/n.ipynb'] });
  });

  it('labels searches by their pattern or query', () => {
    expect(toolDisplay('Grep', { pattern: 'TODO', path: 'src' }))
      .toEqual({ action: 'search', summary: 'TODO', invocation: null, files: [] });
    expect(toolDisplay('Glob', { pattern: '**/*.ts' }))
      .toEqual({ action: 'search', summary: '**/*.ts', invocation: null, files: [] });
    expect(toolDisplay('WebSearch', { query: 'vitest mock timers' }))
      .toEqual({ action: 'search', summary: 'vitest mock timers', invocation: null, files: [] });
  });
  ...
  it('labels a sub-agent by its short description and hands the task text over as the invocation', () => {
    expect(toolDisplay('Agent', { description: 'Audit the docs', prompt: 'Read every page and list gaps.' }))
      .toEqual({ action: null, summary: 'Audit the docs', invocation: 'Read every page and list gaps.', files: [] });
  });

  it('labels a skill load by the skill name', () => {
    expect(toolDisplay('Skill', { skill: 'team-workflow' }))
      .toEqual({ action: null, summary: 'team-workflow', invocation: null, files: [] });
  });
  ...
  it('keeps the action when a known tool arrives with an input it cannot label', () => {
    expect(toolDisplay('Bash', null)).toEqual({ action: 'run', summary: null, invocation: null, files: [] });
    expect(toolDisplay('Read', { file_path: '' })).toEqual({ action: 'read', summary: null, invocation: null, files: [] });
  });
});
