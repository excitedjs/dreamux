...
describe('toolDisplay', () => {
  ...
  it('labels an all-read command by the files it read, deduplicated, the way the TUI groups an Explored cell', () => {
    expect(toolDisplay({
      type: 'commandExecution',
      id: 'item-1',
    ...
    })).toMatchObject({ action: 'read', summary: 'SKILL.md, README.md' });
  });
  ...
  it('labels a search by its query and path', () => {
    ...
    })).toEqual({ action: 'search', summary: 'TODO in src', invocation: 'rg TODO src' });
  });
  ...
  it('labels a patch by the files it touches and shows the diffs codex prepared', () => {
    expect(toolDisplay({
      type: 'fileChange',
      id: 'item-1',
      status: 'completed',
  ...
  it('labels a web search by what was searched, per action kind', () => {
    expect(toolDisplay({ type: 'webSearch', id: 'item-1', query: 'rust async traits', action: null }))
      .toEqual({ action: 'search', summary: 'rust async traits', invocation: null });
    expect(toolDisplay({
      type: 'webSearch',
      id: 'item-1',
      query: 'x',
