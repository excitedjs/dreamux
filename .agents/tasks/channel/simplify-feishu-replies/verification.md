...
At that pre-review checkpoint, independent Devbox review and PR CI had not yet run.
...
claim of review approval. Independent findings are adjudicated against the published review commit. The original source baseline includes an unrelated unmerged
...
the PR's live status.

| Finding | Leader disposition | Evidence and scope | Operator conflict |
| --- | --- | --- | --- |
| N1: plain-text mention keys with a shared prefix can select the wrong identity | Confirmed; defer correction as a non-blocking follow-up | The leader replayed `@_user_1 @_user_10` with two matching records: both positions name the first identity and a literal `0` remains. Baseline `applyMentions` uses the same ordered substring replacement. The new structured projection makes the inherited error an incorrect mention part. No fix was dispatched during this documentation-only integration. | None; this is an inherited defect recorded without claiming corrected behavior. |

The [inline review comment](https://github.com/excitedjs/dreamux/pull/414#discussion_r3990491736)
contains the concrete trigger and proposed single-pass replacement. It remains
a follow-up, not an additional identity-lookup or normalization mechanism.

## Published alpha verification

The operator requested an alpha from the PR head on 2026-09-11.
[Release run 34610506606](https://github.com/excitedjs/dreamux/actions/runs/34610506606)
published `@excitedjs/dreamux@0.26.0-alpha.g87dda034b746` through the existing
branch prerelease workflow. npm initially accepted the matching types package
with an explicit processing notice; metadata and then its tarball became
available later. No duplicate publication or code change was used to bypass
that delay.

An isolated npm install added 127 packages successfully. Installed manifests
confirmed the matching alpha suffix for Dreamux, types, Feishu channel, and
transport. The installed Server and service facade import successfully, and
the installed native-post serializer preserves the authored Markdown. No global
installation, service restart, or Feishu send was performed by that smoke.

The CLI exited successfully but printed `unknown` for `--version`, so the probe's
initial assertion that its output must equal the package version failed. Package
identity was instead verified directly from the installed manifests; the CLI
output is not counted as version evidence. The CLI source was unchanged by this
PR and contains no explicit package-version binding. This limitation is recorded
without broadening the Feishu implementation into a CLI version fix.
