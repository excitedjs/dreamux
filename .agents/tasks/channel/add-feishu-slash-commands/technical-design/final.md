...
- The turn ends through the existing `turn.ended` / `interrupted` path, and the
  ...
  itself but only on a `user` envelope, whose text blocks the COT does not
  display.
...
An accepted interrupt is not a terminal of its own. codex answers it with an
ordinary `turn/completed` whose only distinguishing mark is `turn.status:
"interrupted"` — `error: null`, no items — measured against `codex-cli 0.153.4`
(probe 4 in `verification.md`) and required on the wire since `0.137.0`. So the
provider reads that status onto `CollectedTurn` and pushes the same
`[Request interrupted by user]` assistant message the Claude Code provider does,
ahead of the end it already reports. Reading the status rather than remembering
that we asked matters: `interrupt()` picks a turn with no terminal yet and then
...
| --- | --- | --- |
| `/stop` | the agent this conversation talks to | `team.interrupt` with the bound Team name, or without one when the plan is `dispatcher`; a `provision` plan has no agent yet and is answered without a Command |
