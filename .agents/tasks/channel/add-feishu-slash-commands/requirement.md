...
- `/teams` is not scoped to the asking conversation. Review asked whether it
  should list only Teams this chat can reach; the operator decided the question
  by code size: 「这个按照代码量来决策吧：哪个代码量最少，就选哪个方案。因为这个功能
  完全不投递给Agent，我理解只要艾特了就返回，是不是最简单？」 ("decide this by code
  size: whichever option is least code, take it. Since this feature is never
  delivered to an Agent, my understanding is it just returns as soon as you
  @-mention — isn't that the simplest?"). Unfiltered is the least code — the
  Command's own answer, rendered — so every running Team on the dispatcher is
  listed to any conversation that may reach the bot. This is a ruling, not a
  gap.
- An interrupted Claude Code turn owes the COT one line and nothing more. Asked
  whether the turn should settle as interrupted rather than failed, the operator
  ruled: 「你只要给[Request interrupted by user] 输出到 cot 上，最后卡片是什么状态其实
  没那么重要。一切以对当前架构侵入性最小，为最主要的决策路径」 ("just put [Request
  interrupted by user] on the COT; what state the card ends in doesn't really
  matter. Least intrusion on the current architecture is the primary decision
  path"). The CLI writes that sentence itself, on a `user` envelope whose text
  blocks the COT does not display, so the provider pushes it as an assistant
  message — the shape the operator ruled for `Compacted session`.
- `user` envelopes stay hidden; the provider writes the sentence itself. Asked to
  confirm the shape, the operator ruled: 「user还是保持全都不输出到COT，你自己拼接一下
  这个文案就可以了吧？」 ("keep `user` entirely out of the COT — you just splice this
  text in yourself, right?"). The PR #376 rule that no `user`-envelope text block
  is displayed is therefore untouched.
- A codex interrupt writes the same line. Told that an interrupted codex turn
  ended clean with nothing on the card, the operator ruled: 「codex 也加一个打断text
  ，对齐claude code」 ("add an interrupt text on the codex side too, aligned with
  Claude Code"). Read narrowly, the ruling named only the text, so the first
  implementation left codex's own `completed` end alone.
- Both runtimes end an interrupted turn `interrupted`. The operator tested an
  alpha built from this branch and reported what reading the previous ruling
  narrowly had cost: 「测了一下大部分挺稳的，但是 codex 发送 stop，COT 显示状态是任务
  已完成，Claude code 展示的是任务中断」, then 「应该都是任务中断才对，」 ("mostly
  solid, but sending stop to codex shows 任务已完成 on the COT while Claude Code
  shows 任务中断" … "they should both be 任务中断"). This supersedes 「最后卡片是什么
  状态其实没那么重要」 for the interrupted case and only for it: a card's terminal
  is `turn.ended.status` verbatim, so codex now ends an interrupted turn
  `interrupted`. He did not rule on what a stopped turn delivers, which still
  differs between the runtimes.
- An accepted `/dissolve` answers with nothing. The operator tested the alpha
  and ruled: 「/dissolve 的时候如果成功了，就不发卡。feishu 这边有unbind 卡会自动发出，
  而且我在优化这块了，到时候feishu channel会监听 team closed 或者其他什么事件，发一个
  更好看的解绑卡或者解散卡。现在 /dissolve 发了两张卡了。」 ("on a successful
  /dissolve, send no card. Feishu already sends an unbind card automatically, and
...
- The `/teams` card is a grid of tiles under collapsible per-repository panels.
  The operator called the first implementation ugly, had a design TeamMate send
  him seven rendered candidates, and picked one: 「3 比较好，3 的基础上按照仓库折叠
  起来」 ("3 is better; take 3 and fold it up by repository"). The ruling is about
  layout. It says nothing about copy, so the card stays English under the earlier
  ruling above even though the candidates were rendered in Chinese; that question
  is back with the operator rather than decided here. Candidate JSON is kept in
  the Team workspace under `.workspace/teams-card-candidates/`.
- A Team tile is never truncated to keep the grid even. The first draft shortened
  the intent and replaced all but the first binding with a count, on the guess
  that a tall tile would stretch the one beside it. The operator corrected the
  premise: 「你不用担心，飞书这个两栏做了瀑布布局。这些截断都是无意义的」 ("don't worry,
  Feishu's two columns are a waterfall layout. Those truncations are pointless").
  Both were reverted; the intent keeps the 96-character cap it had before this
  card was redesigned, which is a readability limit and not a layout one.
- The command path stays without an idempotency key. Told that a Feishu
  redelivery would execute a command twice and that a fix means a new
  TTL-expiring seen-set in the Channel, the operator ruled: 「可以先不动。」 ("can
