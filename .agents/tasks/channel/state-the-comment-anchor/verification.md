...
- A selection spanning several blocks records exactly one anchor. The other
  blocks appear nowhere in the item, so no read recovers them.
...
- That a live anchored comment delivers an `anchor_id` a recipient can then
  read back with lark-cli. The probes read the id straight from Feishu and
  fetched the block with it by hand; what is unverified is the round trip
  through a delivered envelope.
