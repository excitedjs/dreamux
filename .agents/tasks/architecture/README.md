...
- [Split streaming display from the push-back mechanism](/.agents/tasks/architecture/split-streaming-display-from-pushback/README.md) — `intake`: Separate the conversation-display surface every channel-facing TeamLeader and Dispatcher needs from the submission push-back mechanism it is currently built on, so a display consumer no longer requires a Dreamux submission to exist.
- [Unify Team command projections](/.agents/tasks/architecture/unify-team-command-projections/README.md) — `review`: One flat Team summary for create and status with lifecycle status as the single status meaning; list keeps a compact row with the same names. PR #390 awaits the operator's re-review of the fix commit.
- [Strengthen independent review prompts](/.agents/tasks/architecture/strengthen-independent-review-prompts/README.md) — `done`: Make solution and implementation-review TeamMates challenge TeamLeader framing and reject entropy-increasing architecture without adding review machinery.
- [Add Team creation context and use the dispatcher workspace by default](/.agents/tasks/architecture/extend-team-create-context/README.md) — `review`: Allow callers to attach provider-owned context to fresh Team creation and default repository-free agents to the dispatcher working directory.
