...
- [Drop lifecycle-stop completion pushback](/.agents/tasks/completion-routing/suppress-owner-close-stop-pushback/README.md) — `delivery`: Stop reporting a turn or Workflow terminal to the owner whose own close, stop, dissolve, or shutdown ended it, by reading the scope fence on the recipient side rather than retiring obligations on the producer side.
