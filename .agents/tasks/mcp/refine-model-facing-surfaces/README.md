...
- Blockers: Open decisions (a) channel clause, (d) dispatch reminder trio, (h) in the requirement; the mid-turn completion-delivery fact is established by probe on both engines.
- Next action: The operator answers the re-asked decisions (a) and (d) on the third question card; the TeamLeader writes the solution for a new PR on `next` that includes PR #369's content (R4).
