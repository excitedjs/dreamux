# Backfilled decision records: cron foundation record
...
- Originating proposal:
  [archived cron-for-TeamLeaders proposal](/.agents/archive/proposals/scheduled-tasks-team-leader.md).
- Baseline it extends:
  [archived scheduled-tasks proposal](/.agents/archive/proposals/scheduled-tasks.md),
  [agent activity capability](/.agents/archive/decisions/agent-activity-capability.md).

### Since this was recorded (2026-09-01)

The core decision stands. Two mechanism names moved on: the host seam is `submitScheduled({jobId, prompt, sourceId})` (`/packages/dreamux/src/service/scheduler/types.ts`), not `scheduledInput`; the `absentRuntimeStrategy` enum no longer exists, and the scheduler contract states that no idle question crosses the call.
