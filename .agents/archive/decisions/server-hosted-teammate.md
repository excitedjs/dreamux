...
> **Archived 2026-09-01** (decisions tree dissolved into task records). Superseded for current implementation by provider-architecture-realignment and later #350. Final destination: [/.agents/tasks/architecture/minimize-provider-boundaries/](/.agents/tasks/architecture/minimize-provider-boundaries/README.md).
...
- **PR / Issue:** [issue #110](https://github.com/excitedjs/dreamux/issues/110);
  supersedes [dispatcher-tm-boundary](/.agents/archive/decisions/dispatcher-tm-boundary.md)
...
`spawn({ agent_runtime: id, ... })` plus whether that runtime supports resume.
Provider refs such as
...
Claude Code Remote Control does not define Dreamux TeamMate turn-delivery
behavior. Dreamux `send` delivery goes through the runtime inbox contract;
Remote-Control-driven external UI turns have separate ownership/attribution
