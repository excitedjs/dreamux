...
> **Archived 2026-09-01** (decisions tree dissolved into task records). Superseded in mechanism by #350: the binding store moved into the Channel and the `binding.*` core event kinds do not exist (`removed-surfaces.test.ts` locks this); the Feishu notification cards survive as Channel-internal behavior.
