...
| 4 | Blocks with a non-null `parent_tool_use_id` (a subagent's) are displayed as the agent's own. | The field exists exactly to tell them apart. The headless guide narrows what arrives: by default only a subagent's `tool_use` and `tool_result` blocks are emitted; its text needs `--forward-subagent-text`, which Dreamux does not pass. A 2.1.272 probe contradicts that for a background subagent: its `thinking` and text arrived too, after the main `result`; a foreground subagent's did not. Every subagent `assistant` and `user` envelope carried the spawning `Agent` call's id; every main-agent envelope carried an explicit `null`. | **Promoted, fixed.** The display projection drops every envelope with a non-null `parent_tool_use_id`; the main agent's own `Agent` call and result stay. Operator ruling 「在 Provider 这层，Subagent 的 Activity 完全不需要上报」, task [hide-claude-subagent-activity](/.agents/tasks/channel/hide-claude-subagent-activity/requirement.md). Left as observed: `TurnAggregator` still reads a background subagent's text into its fallback `lastAssistantText`, used only when a `result` carries no text. |
...
- Subagents (headless guide, "Follow subagent messages"): "Messages from
  ...
  text and thinking blocks." This narrowed row 4; the 2.1.272 probe recorded
  there shows the default does not hold for a background subagent.
