...
must use `ask_user_question` to clarify each issue separately. An omission is not
...
Ask in the originating topic, with its `chat_id` and `message_id`.
...
2. Explain the concrete behavior or scope difference, then ask one focused
   question through `ask_user_question`. Do not bundle independent decisions.
