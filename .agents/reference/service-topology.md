...
| `CollaborationSpaceService` | Constructed by `DispatcherService` at `/packages/dreamux/src/service/dispatcher-service/index.ts:207`. Facade class lives in `/packages/dreamux/src/service/collaboration-space/index.ts:43`; target lifecycle worker lives in `/packages/dreamux/src/service/collaboration-space/target-lifecycle.ts:57`. | Collaboration-space bind/dissolve/status/list facade, collaboration-space records, and delegation to target provisioning/close orchestration. `CollaborationTargetLifecycle` owns target-level locks and target state transitions. | One dispatcher-local service plus one contained target-lifecycle worker. | Depends on `ChannelService`, the dispatcher-level `TeamCollection`, dispatcher config, and `CollaborationSpaceStore`. It owns no agent runtime and does not construct `TeammateService`; provisioned Teams go through `TeamCollection`. |
